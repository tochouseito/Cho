...
void RenderSystem::ObjectRender(PSOMode psoMode, EntityManager& entityManager, ComponentManager& componentManager, D3DCommand* d3dCommand, ResourceViewManager* rvManager, DSVManager* dsvManager,GraphicsSystem* graphicsSystem, TextureLoader* texLoad)
{
    ...
    for (Entity entity : entityManager.GetActiveEntities()) {
        ...
        if (render.wireframe) {
            ...
            // これ何？？
            /*else if (psoMode == PSOMode::GBufferMix)
            {
                commandList->SetGraphicsRootSignature(graphicsSystem->GetPipeline()->GetPSO(PSOMode::WireframeMix).rootSignature.Get());
                commandList->SetPipelineState(graphicsSystem->GetPipeline()->GetPSO(PSOMode::WireframeMix).Blend[kBlendModeNone].Get());
            }*/
        }
